...
if(!item){notice.textContent='Игра не найдена в demo-каталоге.';notice.classList.add('error')}else{
 ...
 window.EJS_player='#game'; window.EJS_core='segaMD'; window.EJS_gameName=item[0]; window.EJS_gameUrl=`roms/${item[1]}`; window.EJS_pathtodata='https://cdn.emulatorjs.org/stable/data/'; window.EJS_startOnLoaded=true; window.EJS_language='ru-RU'; window.EJS_threads=false; window.EJS_color='#d99a47'; window.EJS_backgroundColor='#081018';
 const s=document.createElement('script'); s.src='https://cdn.emulatorjs.org/stable/data/loader.js'; s.onerror=()=>{notice.textContent='Не удалось загрузить EmulatorJS CDN.';notice.classList.add('error')}; document.body.appendChild(s);
}